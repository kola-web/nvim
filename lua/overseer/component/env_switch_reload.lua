-- overseer 自定义组件：环境切换任务成功后自动 reload 被修改的 buffer
-- （app.ts / project.config.json）
-- 通过 runtimepath 发现（nvim_get_runtime_file 扫描所有 rtp 的 lua/overseer/component/*.lua）
local STATUS = require('overseer.constants').STATUS

---@type overseer.ComponentFileDefinition
return {
  desc = '环境切换成功后刷新 app.ts / project.config.json buffer',
  constructor = function()
    return {
      on_complete = function(_, task, status)
        ...
        local meta = task and task.metadata
